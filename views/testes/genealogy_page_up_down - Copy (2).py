...
def _branch_sankey(
    ...
    ncolors = (PALETTE * reps)[: len(nodes)]
        # ---- NOVO: mapa do "primeiro filho" (first-hop) para o lado direito ----
    from collections import defaultdict, deque as _deque
    children_map = defaultdict(list)
    for a, b in edges:
        # só propagamos no lado direito (níveis > 0)
        if level.get(a, 0) >= 0 and level.get(b, 0) > 0:
            children_map[a].append(b)

    firsthop = {}  # nó -> primeiro filho do root no caminho root→nó
    ...
    # Tons (cinzentos) por ramo de 1º nível no lado direito
    TONE_ROOT_DIRECT   = "rgba(0,0,0,0.22)"  # ex.: New Wave → algo
    ...
    # Azul normal (para o caminho focado) e azul semitransparente (para a esquerda)
    BLUE_LEFT = "rgba(59,130,246,0.55)"  # transparência pedida para upstream
    ...
    for a, b in edges:
        if a in idx and b in idx:
            ...
            if on_path:
                # caminho focado continua azul sólido
                lcol.append(BLUE)
            elif is_left_edge:
                # 🔵 esquerda (upstream): azul com alguma transparência
                lcol.append(BLUE_LEFT)
            else:
                # ➜ lado direito (downstream): tons de cinzento por ramo de 1º nível
                if level.get(a, 0) == 0 and level.get(b, 0) == 1:
                    # ligação direta do root
                    ...
                    tone = BRANCH_TONES.get(fh, LINK_GREY)
                    lcol.append(tone)
                else:
                    # fallback (inclui caso branch_only=True)
...
3. In the map you can choose the **depth** (levels below the genre) and select the **branch** step by step.
    ...
    # usar exatamente a mesma fonte de dados do Sankey
    ...
    upstream   = set(parents or [])      # <- já são listas
    downstream = set(children or [])

    n_infl = len(upstream)               # nº de influências diretas
    n_der  = len(downstream)             # nº de derivações diretas

    # --- depois de calcular n_infl / n_der e dos sets `upstream` e `downstream` ---

    show_upstream_preview = (len(upstream) > 0) and (len(downstream) < 8)

    if show_upstream_preview:
        # construir nós/ligacoes só com os pais -> género
        prev_edges = [(p, genre, 1.0) for p in sorted(upstream)]
        prev_nodes = list({genre, *upstream})

        # destacar o género selecionado (mantém foco)
        highlight_nodes = {genre}
        highlight_edges = set()  # se quiseres, podes preencher depois com o caminho ativo
    ...
    # Fallback “1-hop” (pais e filhos diretos) se não houver arestas
    if not edges:
