...
# pass 1: exact on parent labels
for pref in children.keys():
    if pref and norm(pref[-1]) == target:
        candidates.append(pref)

# pass 2: partial on parent labels
if not candidates:
    for pref in children.keys():
        if pref and target in norm(pref[-1]):
            candidates.append(pref)

# pass 3: match on child labels; extend prefix
if not candidates:
    ...
    # pass 4: roots list (top-level)
    if not candidates and roots:
        for r in roots:
            if norm(r) == target or target in norm(r):
                candidates.append((r,))
...
def render_influence_map_page():
    ...
    root_default  = st.session_state.get("infl_root", "")
...
# (o radio já atualiza esta key por nós)
    ...
    st.session_state.setdefault("infl_root", st.session_state.get("infl_root", ""))
