...
# Reduce global link thickness (1.0 = original)
LINK_SCALE = 0.550  # (uniform scaling doesn't visually thin links due to normalization)
# Auto-height tuning (affects visual thickness of links)
AUTO_H_BASE = 460
AUTO_H_PER_BRANCH = 26
AUTO_H_MIN = 420
AUTO_H_MAX = 1800
...
def _auto_height_by_breadth(nodes, links, root_label,
                            base_h=420, px_per_branch=60,
                            h_min=420, h_max=1600):
    """Estimate height from the maximum number of parallel branches at any level."""
...
def _graph_from_csv(root_label: str, down_depth: int = 3, up_levels: int = 1,
    ...
    # pass 1: exact match on parent labels (prefix last segment)
    for pref in children.keys():
        if pref and norm(pref[-1]) == target:
            candidates.append(pref)

    # pass 2: partial match on parent labels
    if not candidates:
        for pref in children.keys():
            if pref and target in norm(pref[-1]):
                candidates.append(pref)

    # pass 3: match on child labels; if found, extend the prefix path
    if not candidates:
    ...
    # pass 4: if still none, return single node (graceful)
    if not candidates:
        return [root_label], []  # single node; caller handles empty edges
...
def _build_sankey(nodes, links, title="Influence map"):
    ...
    # Apply global link-thickness scaling
    val = [v * LINK_SCALE for v in val]
...
# --- Quick picks ---
    ...
    with col_plot:
        ...
        h = _auto_height_by_breadth(nodes, links, root_for_depth)
