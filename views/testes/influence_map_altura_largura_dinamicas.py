...
# (o radio já atualiza esta key por nós)
    ...
    # --- Dynamic height & width (auto) ---
    from collections import defaultdict, deque
    _graph_tmp = defaultdict(list)
    for _s, _t, *_ in links:
        _graph_tmp[_s].append(_t)
    _levels_tmp = {0: 1}
    _q = deque([(root_for_depth, 0)])
    _seen = {root_for_depth}
    _MAX_DEPTH = 0
    while _q:
        _u, _d = _q.popleft()
        for _v in _graph_tmp.get(_u, []):
            if _v not in _seen:
                _seen.add(_v)
                _q.append((_v, _d + 1))
                _levels_tmp[_d + 1] = _levels_tmp.get(_d + 1, 0) + 1
                if _d + 1 > _MAX_DEPTH:
                    _MAX_DEPTH = _d + 1
    _max_breadth = max(_levels_tmp.values()) if _levels_tmp else 1
    _AUTO_H = max(420, min(100 + 60 * _max_breadth, 1600))
    _AUTO_W = max(640, min(720 + 220 * _MAX_DEPTH, 2000))
    ...
    with col_plot:
        ...
        fig.update_layout(height=_AUTO_H, width=_AUTO_W)
        ...
        if HAS_PLOTLY_EVENTS:
            events = plotly_events(
                ...
                hover_event=False,
                select_event=False,
                override_height=_AUTO_H,
                override_width=_AUTO_W,
